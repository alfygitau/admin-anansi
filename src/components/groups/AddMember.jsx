...
const AddGroupMember = ({
  ...
  // Mock list of active SACCO members for selection lookup
  const registeredSaccoMembers = [
    {
      id: "MEM-4021",
      name: "Sarah Wanjiku",
  ...
  const validateField = (fieldName, value) => {
    ...
    if (fieldName === "memberId") {
      if (!value || !value.trim()) {
        errorMessage = "Please select a registered SACCO member.";
      }
    }

    if (fieldName === "groupRole") {
      if (!value || !value.trim()) {
        errorMessage = "Please select a group role for this member.";
      }
    }

    if (fieldName === "monthlyCommitment") {
      if (!value || isNaN(value) || Number(value) <= 0) {
        errorMessage = "Please enter a valid monthly contribution target.";
      }
    }
  ...
  const handleProceedToPreview = () => {
    ...
    const groupRoleError = validateField("groupRole", formData.groupRole);
    const commitmentError = validateField(
      "monthlyCommitment",
      formData.monthlyCommitment,
    );

    if (memberIdError || groupRoleError || commitmentError) return;
    setStep("preview");
  };
  ...
  const handleClearAllStates = () => {
    setStep("form");
    setErrors({ memberId: "", groupRole: "", monthlyCommitment: "" });
    setFormData({
      memberId: "",
      memberName: "",
  ...
  const filteredMembers = registeredSaccoMembers.filter(
    (m) =>
            ...
            {step === "form" && (
              <>
                ...
                <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
                    ...
                    {formData.memberId ? (
                      ...
                      <div className="space-y-2">
                        <div className="relative">
                          ...
                          <input
                            type="text"
                            placeholder="Search by name or ID (e.g., Sarah, MEM-4021)"
                            value={searchQuery}
                        ...
                        <div className="max-h-48 overflow-y-auto border border-slate-100 rounded-2xl divide-y divide-slate-100 bg-slate-50/50">
                          {filteredMembers.length > 0 ? (
                            ...
                            <div className="p-4 text-center text-xs text-slate-400 font-medium">
                              No active individual SACCO members found.
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  {/* STEP 1B: GROUP ROLE ASSIGNMENT */}
                  <FilterField
                    label="Group Operational Role"
                    icon={Shield}
                    error={errors.groupRole}
                  >
                    <select
                      value={formData.groupRole || "Regular Member"}
                      onChange={(e) => {
                        setFormData({ ...formData, groupRole: e.target.value });
                        if (errors.groupRole)
                          setErrors({ ...errors, groupRole: "" });
                      }}
                  ...
                  {/* STEP 1C: MONTHLY COMMITMENT */}
                  <FilterField
                    label="Monthly Contribution Commitment (KES)"
                    icon={DollarSign}
                    error={errors.monthlyCommitment}
                  >
                    <input
                      type="number"
                      ...
                      onChange={(e) => {
                        ...
                        if (errors.monthlyCommitment)
                          setErrors({ ...errors, monthlyCommitment: "" });
                      }}
                ...
                <div className="p-8 py-5 border-t border-slate-100 flex gap-3 select-none bg-white">
                  ...
                  >
                    <span>Review Member Details</span>
                    <ArrowRight size={14} />
                  </button>
                </div>
              </>
            ...
            {step === "preview" && (
              <>
                <div className="px-8 pt-5 pb-6 select-none">
                  ...
                  <p className="text-xs text-slate-400 font-medium mt-0.5">
                    Confirm member allocation and financial commitments before
                    updating the group register.
                  </p>
                </div>
                <div className="border-b mx-8 border-slate-100"></div>
                ...
                <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
                  ...
                  <div className="bg-blue-50/40 border border-blue-100/60 rounded-xl p-4 flex items-center gap-3 select-none">
                    ...
                    <p className="text-[11px] font-medium text-[#074073]/80 leading-normal">
                      Adding this member will automatically link their main
                      SACCO balance logs to this group's collective portfolio
                      dashboard.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-4 max-w-sm mx-auto">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400 font-medium">
                      The verified individual has been successfully mapped to
                      the group portfolio.
                    </p>
                  </div>
