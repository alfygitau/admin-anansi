...
export default function RepayLoan({
  ...
  const handlePay = async () => {
    ...
    if (!cleanPhone || !/^(?:\+?254|0)[17]\d{8}$/.test(cleanPhone)) {
      setError("Please provide a valid M-Pesa phone number (e.g. 0712345678 or 254712345678).");
      return;
    }

    if (!totalAmount || totalAmount <= 0) {
      setError("Please enter a valid loan repayment amount.");
      return;
    }
    ...
    try {
      const payload = {
        loan_id: loan?.id || paymentDetails?.loanId,
        ...
        <div className="fixed inset-0 z-[100] flex justify-end bg-zinc-950/20 font-sans antialiased text-slate-800">
          ...
          >
            ...
            <div className="px-8 pt-6 pb-4 flex items-center justify-between border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium mt-0.5">
                  Repay outstanding loan balance directly via M-Pesa STK push.
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 py-4 space-y-5">
              ...
              {/* LOAN INFO CARD (Shows facility info if provided) */}
              {loan && (
                <div className="p-4 bg-slate-50 border border-slate-200/80 rounded-2xl flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs font-bold text-slate-800 font-mono">
                      {loan.loan_code || loan.reference || "Loan Account"}
                    </p>
                  </div>
                  <div className="text-right space-y-0.5">
                    ...
                    <p className="text-xs font-black text-rose-600 font-mono">
                      {formatAmount(loan.outstanding_balance || loan.balance || 0)}
                    </p>
                  </div>
                </div>
              )}
